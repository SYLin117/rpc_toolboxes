...
class SRM_Unet(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(SRM_Unet, self).__init__()
        self.srm = SRMConv(in_channels=n_channels, out_channels=3)
        self.inc = inconv(n_channels + 3, 64)
        self.down1 = U_down(64, 128)
        self.down2 = U_down(128, 256)
        self.down3 = U_down(256, 512)
        self.down4 = U_down(512, 512)
...
class SRM_Res_Unet(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(SRM_Res_Unet, self).__init__()
        self.srm = SRMConv(in_channels=n_channels, out_channels=3)
        self.down = RU_first_down(n_channels + 3, 32)
        self.down1 = RU_down(32, 64)
        self.down2 = RU_down(64, 128)
        self.down3 = RU_down(128, 256)
        self.down4 = RU_down(256, 256)
...
class SRM_Ringed_Res_Unet(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        super(SRM_Ringed_Res_Unet, self).__init__()
        self.srm = SRMConv(in_channels=n_channels, out_channels=3)
        self.down = RRU_first_down(n_channels + 3, 32)
        self.down1 = RRU_down(32, 64)
        self.down2 = RRU_down(64, 128)
        self.down3 = RRU_down(128, 256)
        self.down4 = RRU_down(256, 256)
