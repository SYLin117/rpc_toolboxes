...
def thread_find_obj_loc(bg_img, mask_img, category, ratio_annotations, json_ann, obj_in_this_pic, ann_idx):
    ...
    if save_mask:
        ...
        erosion = cv2.erode(mask_cv2, kernel, iterations=5)
        ...
        for cnt in Contours:
            ...
            cv2.drawContours(color_mask_cv2, [hull], -1, color=(0, 0, 0), thickness=20, )
...
if __name__ == '__main__':
    ...
    # ---------------------------
    # store image name to dict with key:catogory, value:list(path)
    # ---------------------------
        ...
        for category, count in num_per_category.items():
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for _ in range(count):
                    ann_idx += 1
                    future = executor.submit(thread_find_obj_loc, bg_img, mask_img, category, ratio_annotations,
                                             json_ann, obj_in_this_pic, ann_idx)
                    futures.append(future)
                cnt_idx = 0
                for future in as_completed(futures):  # make sure all object in this image are found
                    cnt_idx += 1
                    # print('{}/{} object create'.format(cnt_idx, count))
