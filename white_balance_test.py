...
if __name__ == "__main__":
    ...
    # img5: 动态阈值算法
    # '''
    # img = cv2.imread(r"D:\datasets\retail_product_checkout\val2019\20181010-16-10-44-2946.jpg")
    ...
    # img6 = auto_whiteBalance(img).astype(np.uint8)
    # print('----------------------')
    #
    # # img_stack = np.hstack([img, img1, img2, img3, img4, img5, img6])
    # img_stack = np.hstack([img, img5, img6])
    # # cv2.imwrite("./dataset/"+str(i)+'.JPG',img_stack)
    # cv2.imshow('image', img_stack)
    # cv2.waitKey(0)

    folder = r'D:\datasets\retail_product_checkout\val2019'
    save_folder = r'D:\datasets\retail_product_checkout\val2019(auto_balance)'
    ...
    for file in tqdm(files):
        ...
        img = cv2.imread(file)
        # img = cv2.resize(img, (512, 512), cv2.INTER_CUBIC)
        white_balance_img = white_balance_5(img)
        cv2.imwrite(os.path.join(save_folder, filename), white_balance_img)
